# SUMMARY-15-06-13.R from GAMLSS 4.2-6
# Fixes errors if mu.coef.table is missing, see lines 253 and 296
...
#================ mu ESTIMATES ========================
    if ("mu"%in%object$parameters)
    {
        ...
        if (exists("mu.coef.table")) coef.table <- mu.coef.table
    }
    else
    {
        if (df.r > 0) {
    ...
    if ("sigma"%in%object$parameters)
    {
        ...
        cat("\n")
         ...
         ifelse(exists("mu.coef.table"), coef.table <- rbind(mu.coef.table, sigma.coef.table), coef.table <- sigma.coef.table)
    }
    if ("nu"%in%object$parameters)
    {
         if (object$nu.df != 0)
